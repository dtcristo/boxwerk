...
module Boxwerk
  class VisibilityCheckerTest < Minitest::Test
    def test_enforces_visibility_when_true
      pkg = Package.new(name: 'packs/a', config: { 'enforce_visibility' => true })
      assert VisibilityChecker.enforces_visibility?(pkg)
    end

    def test_does_not_enforce_visibility_when_false
      pkg = Package.new(name: 'packs/a', config: { 'enforce_visibility' => false })
    ...
    def test_visible_when_accessor_in_visible_to
      target = Package.new(
        name: 'packs/a',
        config: { 'enforce_visibility' => true, 'visible_to' => ['packs/b'] },
      )
      accessor = Package.new(name: 'packs/b', config: {})
