...
class ZeitwerkScannerTest < Minitest::Test
  ...
  def test_scan_deeply_nested
    ...
    write_file(
      'billing/processors/stripe.rb',
      'module Billing; module Processors; class Stripe; end; end; end',
    )
  ...
  def test_build_file_index
    ...
    assert_equal File.join(@tmpdir, 'billing/payment.rb'),
                 index['Billing::Payment']
  ...
  def test_register_autoloads_nested_in_box
    ...
    write_file(
      'billing/payment.rb',
      'module Billing; class Payment; def self.charge = 100; end; end',
    )
